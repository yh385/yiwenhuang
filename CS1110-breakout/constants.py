...
This module global constants for the game Breakout. These constants need to be
...
PADDLE_OFFSET = 30
#: the distance the paddle moves by each time a left or right arrow key is pressed
ANIMATION_STEP = 10
#: the value of x1 so that the paddle is in the center of the window
PAD_MIDPOINT  = 240
...
#: the list of brick colors from top to bottom
BRICK_COLORS   = (([colormodel.RED]*2)+([colormodel.ORANGE]*2)+
...
STATE_ACTIVE    = 4
#: state when user wins the game
STATE_COMPLETE  = 5
#: state when user loses the game
STATE_LOST      = 6
...
arguments are everything after the work python. Sp if you start the game typing

    python breakout.py 3 4
    
Python puts ['breakout.py', '3', '4'] into sys.argv. Below, we take advantage of
this fact to change the constants BRICKS_IN_ROW and BRICK_ROWS
"""
